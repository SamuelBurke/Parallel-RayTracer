...
glm::vec3 Sphere::Shade(std::shared_ptr<Ray> _ray, RayTracer& _rayTracer, glm::vec3(_intersection), int _count)
{
	...
	//glm::vec3 lightDirection = glm::vec3(220.0f, 420.0f, 0.0f) - _intersection; // The vector here is the light's origin point. -pi in order to get the direction of the light.
	...
	//colour = (mColour + glm::vec3(1.0f) * dt) * 0.5f + reflectColour;// *mColour;
